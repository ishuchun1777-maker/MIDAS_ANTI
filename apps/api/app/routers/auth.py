...
def verify_telegram_authorization(data: TelegramLoginData) -> bool:
    ...
    Faqat Telegram tomonidan yuborilgan fieldlarni ishlatadi.
    """
    received_hash = data.hash
    if not received_hash:
        return False

    # Faqat None EMAS bo'lgan fieldlarni olish (hash bundan tashqari)
    fields = {
        "id": data.id,
    ...
    logger.info(f"Hash check string fields: {[f.split('=')[0] for f in data_check_arr]}")
